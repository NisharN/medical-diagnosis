#intializing pinecone for vectorstore, upload file and chunk it index every module and store those data into vector database and store metadata into mongodb
import os
import time
import asyncio #sync multiple api calls
from pathlib import Path #to get file path
   ...
   metadatas=[
       {
           ...
           "text":chunk.page_content[:2000]#store snippet in metadata
       }
       for chunk in chunks
   ]
    #embed texts
   embeddings=await asyncio.to_thread(embed_model.embed_documents,texts)
   #upset-run in thread to avoid blocking
   def upsert():
